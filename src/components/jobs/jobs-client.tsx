    ...
    <div className="mx-auto w-full max-w-[1120px] px-4 py-5 md:px-6 md:py-7">
      ...
      {showCarousel && (
        <div className="border-border bg-card mb-8 overflow-hidden rounded-xl border shadow-sm">
          <div className="flex items-center justify-between px-5 pt-5 pb-3">
            ...
            <div className="flex gap-1.5">
              <button
                ...
                aria-label="이전"
                onClick={scrollPrev}
                className="border-border hover:border-point flex size-8 cursor-pointer items-center justify-center rounded-lg border transition-colors active:scale-95"
              >
          ...
          <div
            className="w-full overflow-hidden px-4 pt-1 pb-5 select-none sm:px-5"
            ref={emblaRef}
          >
            <div className="flex items-stretch gap-4">
              {carouselItems.map((o) => (
                <div
                  key={o.id}
                  className="min-w-0 flex-[0_0_280px] shrink-0 sm:flex-[0_0_310px]"
                >
                  <JobCard job={o} />
                </div>
              ))}
...
function FilterChip({
  ...
  return (
    <button
      ...
      onClick={onClick}
      className={cn(
        "cursor-pointer rounded-lg px-3 py-1.5 text-[13px] font-bold transition-colors",
        active
      ...
      {paginationRange.map((p, idx) => {
        ...
        return (
          <button
            ...
            onClick={() => go(pageNum)}
            className={cn(
              "h-8 min-w-8 cursor-pointer rounded-[10px] px-2 text-xs font-bold tabular-nums transition-colors sm:h-9 sm:min-w-9 sm:text-sm",
              pageNum === page
                ? "bg-primary text-primary-foreground"
                : "border-border bg-card text-muted-foreground hover:border-point hover:text-point-hover border",
            )}
...
function PagerArrow({
  ...
  return (
    <button
      ...
      disabled={disabled}
      onClick={onClick}
      className="border-border bg-card text-muted-foreground hover:border-point hover:text-point-hover flex size-8 cursor-pointer items-center justify-center rounded-[10px] border transition-colors disabled:pointer-events-none disabled:opacity-40 sm:size-9"
    >
      {children}
    </button>
  );
